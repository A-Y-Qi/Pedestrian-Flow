...
#include "WallRepForce2.h"

#define pedestrian 50
...
int main()
{
    ...
    double wall3_start[2] = {25, 0};
    double wall3_end[2] = {25, 3};
    double wall4_start[2] = {25, 7};
    double wall4_end[2] = {25, 10};
    ...
    outwall1 = fopen("General_wall3.xyz", "w");
    outwall2 = fopen("General_wall4.xyz", "w");
    ...
    outpos1 = fopen("Wall_with_hole.xyz", "w");
    outpos2 = fopen("Wall_with_hole.dat", "w");
    ...
    for (t = 0; t < 100; t += dt)
    {
        ...
        for (i = 0; i < pedestrian; i++)
        {
            if (i <= (int)floor(pedestrian / 2) && X[i] < 25)
            {
                if (Y[i] >= 6.9)
                {
                    e_x[i] = 25 - X[i];
            ...
            else if (i > (int)floor(pedestrian / 2) && X[i] > 25)
            {
                if (Y[i] >= 6.9)
                {
                    e_x[i] = 25 - X[i];
            ...
            Fsum_wall_x[i] = F_wall1_x[i] + F_wall2_x[i] + F_wall3_x[i] + F_wall4_x[i];
            Fsum_wall_y[i] = F_wall1_y[i] + F_wall2_y[i] + F_wall3_y[i] + F_wall4_y[i];
